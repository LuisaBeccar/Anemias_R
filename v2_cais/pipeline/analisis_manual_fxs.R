...
analizando_resultados_manual <- function(tabla) {
  ...
  resumen_counts <- list(
    n_rows = nrow(tabla),
    n_dni = n_distinct(tabla$dni),
    n_nombres = n_distinct(tabla$nombre),
    n_nom_dni = n_distinct(tabla$nombre, tabla$dni),
  ...
  resumir_tabla <- tabla %>% 
    #mutate(comentario_normalizado = str_remove(comentario, ",.*$")) %>% 
    select(-c(origen, cama, nombre, dni, nro_hc, f_internacion, fi_clinica_medica, f_hb_inicial, comentario)) %>% 
...
generar_reporte_calidad <- function(tabla, extraction_log) {
  ...
  duplicates_stats <- tabla %>%
    ...
    summarise(
      n_duplicados = n(),
      archivos = paste(origen, collapse = ", ")
    )
